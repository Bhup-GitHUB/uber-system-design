...
func (qt *QuadTree) Insert(driver *common.Driver) bool {
	qt.mu.Lock()
	defer qt.mu.Unlock()
	return qt.root.insert(driver)
}

func (node *QuadTreeNode) insert(driver *common.Driver) bool {
...
func (qt *QuadTree) SearchRadius(lat, lng, radiusKm float64) []*common.Driver {
	qt.mu.RLock()
	defer qt.mu.RUnlock()
	...
	results := make([]*common.Driver, 0)
	qt.root.searchInBoundary(&searchBox, &results)

	filtered := make([]*common.Driver, 0)
...
func (node *QuadTreeNode) searchInBoundary(searchBox *BoundingBox, results *[]*common.Driver) {
